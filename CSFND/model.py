...
class HiModel(nn.Module):
    ...
    def average_cal(self, tri_feats, emb_feats, clu_ids, modal):
        """
        对于每一个数据，用他同cluster的16个其他数据的features求均值。
        不足16个的用数据0补齐，超过的随机舍弃。
        ...
        selected_neighbor_features = torch.zeros(batch_size, nei_number, tri_feats.size(1)).cuda()  # [batch_size, nei_number, feature_dim]
        for i in range(batch_size):
            neighbor_features = tri_feats[mask[i], :]
            neighbor_count = len(neighbor_features)
            if neighbor_count <= nei_number:
                selected_neighbor_features[i, :neighbor_count, :] = neighbor_features
            else:
                # 超出nei_number个同类数据，随机选择若干数据
                random_neighbors = np.random.choice(neighbor_count, nei_number, replace=False)
                selected_neighbor_features[i, :, :] = neighbor_features[random_neighbors, :]

        # Calculate the mean of the selected neighbor features
        cluster_features = selected_neighbor_features.mean(dim=1)  # [batch_size, feature_dim]
    ...
    def forward(self, input_t, input_v, te_clu_ids, im_clu_ids, t_tri_en, v_tri_en):
        ...
        if self.args.avg :
            # 使用平均值得到 平均的上下文信息
            print("use the average context infomation")
            te_clu_emb, _ = self.average_cal(t_tri_en, None, te_clu_ids, 'text')
            im_clu_emb, _ = self.average_cal(v_tri_en, None, im_clu_ids, 'image')
        elif self.args.agg:
            print("use the attention agg context information")
        ...
        else:# 不使用 AGG 和 AVG 只使用自己当前的
            print("only use the self cluster context information")
            te_clu_emb = t_tri_en
            im_clu_emb = v_tri_en
        ...
        if not self.args.unspr: # 如果不使用聚类的上下文信息和多模态特征融合。
            print("we DO NOT fusion the context information and the multimodal feature ")
            mm_fused = self.cross_attention(input_v, input_t, attn_mask=None)        
        else:
